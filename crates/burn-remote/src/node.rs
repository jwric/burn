...
impl RemoteNode {
    /// Bind a node using Iroh's production relay and address-lookup defaults.
    ...
    /// Create a remote device from control-plane-issued connection material.
    #[cfg(feature = "client")]
    pub fn device_from_ticket(
        &self,
        ticket: &crate::RemoteTicket,
        device_index: usize,
    ) -> crate::RemoteDevice {
        self.device_authorized(
            ticket.peer().clone(),
            device_index,
            ticket.authorization().to_vec(),
        )
    }
