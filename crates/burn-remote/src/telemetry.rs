...
fn classify_base(op: &burn_ir::BaseOperationIr) -> OpClass {
    ...
    match op {
        B::Reshape(_) | B::SwapDims(_) | B::Permute(_) | B::Flip(_) | B::Expand(_)
        | B::RepeatDim(_) | B::Cat(_) | B::Unfold(_) => OpClass::Reshape,
        B::Cast(_) => OpClass::Cast,
        B::Empty(_) | B::Ones(_) | B::Zeros(_) => OpClass::Init,
        B::Equal(_) | B::EqualElem(_) | B::NotEqual(_) | B::NotEqualElem(_) | B::All(_)
        | B::Any(_) | B::AllDim(_) | B::AnyDim(_) => OpClass::Compare,
        _ => OpClass::Index,
    }
}
...
/// A peer's connection as seen from this node, sampled from the live Iroh paths.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerLink {
    /// Remote endpoint identity.
    pub peer: String,
    /// Whether the selected path is a direct IP path rather than a relay.
    pub direct: bool,
    /// Round-trip time of the selected path, in milliseconds.
    pub rtt_ms: Option<f32>,
    pub bytes_sent: u64,
    pub bytes_recv: u64,
}

/// Snapshot of every peer connection this node currently holds.
